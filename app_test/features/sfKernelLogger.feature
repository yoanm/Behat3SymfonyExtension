...
Feature: SfKernelLogger
  ...
  Scenario: Request event
    ...
    And A log entry must exist for symfony app request event to valid route
  ...
  Scenario: Exception event
    ...
    And A log entry must exist for symfony app request event to exception route
    And A log entry must exist for symfony app exception event
